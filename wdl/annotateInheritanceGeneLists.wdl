...
workflow AnnotateInheritanceGeneLists {
    ...
    scatter (vcf_shard in vep_vcf_files) {
        ...
        call annotateInheritanceGeneLists {
            input:
                vcf_file=vcf_shard,
                annotate_inheritance_gene_lists_script=annotate_inheritance_gene_lists_script,
        ...
        call helpers.addGenotypes as addGenotypes {
            input:
            annot_vcf_file=annotateInheritanceGeneLists.annot_vcf_file,
            vcf_file=vcf_shard,
            genome_build=genome_build,
            hail_docker=hail_docker,
...
task annotateInheritanceGeneLists {
    input {
        File vcf_file
        File inheritance_uri
        ...
        String hail_docker
        String genome_build
        String annotate_inheritance_gene_lists_script
    ...
    String vep_annotated_vcf_name = "~{prefix}.annot.inheritance.gene.lists.vcf.bgz"

    command <<<
        curl ~{annotate_inheritance_gene_lists_script} > annotate.py
        python3 annotate.py -i ~{vcf_file} -o ~{vep_annotated_vcf_name} --cores ~{cpu_cores} --mem ~{memory} \
        --build ~{genome_build} --inheritance ~{inheritance_uri} --genes ~{gene_list_tsv} 
