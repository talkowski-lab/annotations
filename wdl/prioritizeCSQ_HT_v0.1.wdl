...
workflow prioritizeCSQ {
    ...
    output {
        Array[String] output_prior_csq_ht = annotateMostSevereCSQ.output_prior_csq_ht
    }
}
...
task annotateMostSevereCSQ {
    ...
    command <<<
        ...
        python3 prioritize_csq.py ~{input_ht} ~{cpu_cores} ~{memory} \
            ~{vep_vcf_file} ~{genome_build} ~{bucket_id}
    >>>
